...
class _HttpHeaders implements HttpHeaders {
  ...
  List<String> operator [](String name) => _headers[name]; //MODIFIED

  String value(String name) {
    // name = name.toLowerCase(); //MODIFIED
  ...
  static String _validateField(String field) {
    ...
    return field; //MODIFIED  field.toLowerCase();
  }
  ...
  void _parseSetCookieValue(String s) {
    ...
    void parseAttributes() {
      ...
      while (!done()) {
        ...
        if (name == "expires") {
          expires = _HttpDate._parseCookieDate(value);
